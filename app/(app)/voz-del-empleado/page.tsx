    ...
    <div className="animate-fade pb-12">
      ...
      <div className="grid gap-4 lg:grid-cols-[1fr_1.6fr]">
        <Card className="rounded-xl p-[20px]">
          ...
          <div className="mt-4">
            <div className="mb-1.5 text-micro font-semibold uppercase tracking-wide text-ink-3">{t("byLineTitle")}</div>
            <div className="flex flex-wrap gap-1.5">
              {data.byLine.map((l) => (
                <span
                  key={l.line}
                  className="rounded-md px-2 py-1 font-mono text-micro font-semibold"
        ...
        <Card className="rounded-xl p-[20px]">
          ...
          {digest.data && (
            <>
              ...
              <p className="mt-1 text-copy leading-relaxed text-ink-2">{digest.data.summary}</p>
              ...
              <div className="mt-3 text-micro text-ink-3">
          ...
          {data.alerts.map((a) => {
            ...
            return (
              <div key={a.id} className="flex items-start gap-2.5 rounded-xl border px-3.5 py-3" style={{ borderColor: `${c}40`, background: `${c}0d` }}>
                ...
                <div>
                  <div className="text-copy font-semibold text-ink-1">
      ...
      <div className="mt-4 grid gap-4 lg:grid-cols-[1.3fr_1fr]">
        {/* Temas */}
        <Card className="rounded-xl p-[20px]">
          <h3 className="text-body font-semibold">{t("themesTitle")}</h3>
          <p className="mt-0.5 text-meta text-ink-3">{t("themesSub")}</p>
        ...
        {/* Verbatims */}
        <Card className="rounded-xl p-[20px]">
          <h3 className="text-body font-semibold">{t("verbatimsTitle")}</h3>
          <p className="mt-0.5 text-meta text-ink-3">{t("verbatimsSub")}</p>
...
function Stat({ label, value, color }: { label: string; value: string; color?: string }) {
  return (
    <div>
      <div className="font-mono text-heading font-bold leading-tight" style={{ color: color ?? "#2B2D42" }}>
        {value}
      </div>
      <div className="mt-0.5 text-micro leading-tight text-ink-3">{label}</div>
    </div>
  );
}
    ...
    <div>
      <div className="mb-1 flex items-center justify-between text-copy">
        ...
        <span className="flex items-center gap-2">
          <span className="text-micro text-ink-3">{theme.mentions} {mentionsLabel}</span>
          <Sparkline data={theme.trend} color={c} />
          <span className="flex items-center font-mono text-micro" style={{ color: up ? "#5B6EF5" : "#EB4F6C" }}>
            {up ? <ArrowUpRight className="h-3 w-3" /> : <ArrowDownRight className="h-3 w-3" />}
            {Math.abs(theme.delta)}
          </span>
          <span className="w-9 text-right font-mono text-meta font-bold" style={{ color: c }}>{fmtSent(theme.sentiment)}</span>
        </span>
      </div>
...
function VerbatimCard({ v, channelLabel, themeLabel }: { v: Verbatim; channelLabel: string; themeLabel: string }) {
  ...
  return (
    <div className="rounded-lg border-l-2 bg-surface-bg/50 py-2 pl-3 pr-2.5" style={{ borderColor: c }}>
      <p className="text-copy leading-snug text-ink-1">“{v.text}”</p>
      <div className="mt-1.5 flex items-center gap-2 text-micro text-ink-3">
