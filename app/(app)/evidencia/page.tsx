  ...
  return (
    <div className="animate-fade pb-12">
      <div className="py-5">
        ...
        <p className="mt-1 max-w-2xl text-sm text-ink-2">{t("subtitle")}</p>
      </div>
      ...
      <Card className="rounded-xl p-[22px]">
        <div className="grid gap-6 lg:grid-cols-[1.1fr_1fr] lg:items-center">
          <div>
            <div className="text-[11px] font-semibold uppercase tracking-wide text-ink-3">{t("verdictLabel")}</div>
            <div className="mt-1 flex items-end gap-3">
              <span className="font-mono text-[52px] font-bold leading-none" style={{ color: SOL }}>+{f.number(p.ate)}</span>
              <span className="mb-1.5 text-[15px] font-medium text-ink-2">pp</span>
              <span
                className="mb-2 inline-flex items-center gap-1.5 rounded-full px-2.5 py-1 text-[11.5px] font-semibold"
            ...
            <div className="mt-2 text-[12.5px] text-ink-3">{t("ci", { low: f.number(p.ciLow), high: f.number(p.ciHigh) })}</div>
            <p className="mt-3 max-w-md text-[13.5px] leading-relaxed text-ink-1">
      ...
      <div className="mt-4 grid gap-4 lg:grid-cols-[1fr_1.3fr]">
        <Card className="rounded-xl p-[22px]">
          <h3 className="text-[15px] font-semibold">{t("designTitle")}</h3>
        ...
        <Card className="rounded-xl p-[22px]">
          <h3 className="text-[15px] font-semibold">{t("roiTitle")}</h3>
          <div className="mt-4 grid gap-4 sm:grid-cols-2">
            <div className="rounded-lg border border-line bg-surface-bg/60 p-4">
              <div className="text-[11.5px] text-ink-2">{t("roiPilotLabel")}</div>
              <div className="mt-1 font-mono text-[26px] font-bold leading-tight" style={{ color: SOL }}>{p.extraRetainedPilot}</div>
              <div className="mt-1 text-[12px] text-ink-3">{t("roiCostPilot")}: <span className="font-medium text-ink-1">{formatMxn(p.costAvoidedPilot)}</span></div>
            </div>
            <div className="rounded-lg border border-line bg-surface-bg/60 p-4">
              <div className="text-[11.5px] text-ink-2">{t("roiAnnualLabel")}</div>
              <div className="mt-1 font-mono text-[26px] font-bold leading-tight" style={{ color: VIO }}>{formatMxn(p.costAvoidedAnnual)}</div>
              <div className="mt-1 text-[12px] text-ink-3">{t("roiAnnualSub", { n: f.number(p.annualEligible) })}</div>
            </div>
          </div>
          <p className="mt-3 text-[11.5px] text-ink-3">{t("roiNote", { cost: formatMxn(p.replacementCostMxn) })}</p>
        </Card>
      </div>

      {/* Convergencia del efecto */}
      <Card className="mt-4 rounded-xl p-[22px]">
        <h3 className="text-[15px] font-semibold">{t("trendTitle")}</h3>
        <p className="mt-0.5 text-[12.5px] text-ink-2">{t("trendSub")}</p>
        <TrendChart points={p.trend} xLabel={t("trendX")} yLabel={t("trendY")} />
      </Card>

      {/* Heterogeneidad por línea */}
      <Card className="mt-4 rounded-xl p-[22px]">
        <h3 className="text-[15px] font-semibold">{t("byLineTitle")}</h3>
        <p className="mt-0.5 max-w-2xl text-[12.5px] text-ink-2">{t("byLineSub")}</p>
        <ByLine rows={p.byLine} t={t} f={f} />
      </Card>

      {/* Flywheel del modelo */}
      <Card className="mt-4 rounded-xl p-[22px]">
        <h3 className="text-[15px] font-semibold">{t("flywheelTitle")}</h3>
        <p className="mt-1 max-w-3xl text-[12.5px] leading-relaxed text-ink-2">{t("flywheelBody")}</p>
        <Flywheel retrains={p.retrains} labelsWord={t("flywheelLabels")} projectedWord={t("flywheelProjected")} />
        <div className="mt-4 flex flex-wrap gap-2.5">
          <Link href="/seguimiento" className="inline-flex items-center gap-1.5 rounded-lg border border-line-2 px-3 py-1.5 text-[12.5px] font-medium text-ink-1 transition-colors hover:border-risk-sol hover:text-risk-sol">
            {t("flywheelLinkLoop")} <ArrowRight className="h-3.5 w-3.5" />
          </Link>
          <Link href="/modelo" className="inline-flex items-center gap-1.5 rounded-lg border border-line-2 px-3 py-1.5 text-[12.5px] font-medium text-ink-1 transition-colors hover:border-risk-sol hover:text-risk-sol">
            {t("flywheelLinkModel")} <ArrowRight className="h-3.5 w-3.5" />
          </Link>
        </div>
      </Card>

      <p className="mt-4 max-w-3xl text-[11.5px] leading-relaxed text-ink-3">
...
function Field({ label, value, sub }: { label: string; value: string; sub?: string }) {
  return (
    <div>
      ...
      <div className="mt-1 text-[15px] font-medium text-ink-1">{value}</div>
  ...
  return (
    <div>
      <div className="flex items-baseline justify-between">
        ...
        <span className="font-mono text-[15px] font-bold" style={{ color }}>{rate.toFixed(1)}%</span>
      </div>
      ...
      {rows.map((r) => {
        ...
        return (
          <div key={r.line} className="flex items-center gap-3">
            <span className="w-7 shrink-0 font-mono text-[12px] font-semibold text-ink-1">{r.line}</span>
            <span className="w-24 shrink-0 text-[10.5px] text-ink-3">{t("byLineCount", { nt: r.nT, nc: r.nC })}</span>
            ...
            <span className="w-12 shrink-0 text-right font-mono text-[12px] font-semibold" style={{ color }}>
              {r.uplift > 0 ? "+" : ""}{f.number(r.uplift)}
            </span>
            {!sig && <span className="w-[88px] shrink-0 text-[10px] text-ink-3">{t("notSignificant")}</span>}
        ...
        <div key={r.version} className="flex items-center gap-2">
          <div className={`min-w-[124px] rounded-lg border p-3 ${r.projected ? "border-dashed border-line-2 bg-transparent" : "border-line bg-surface-bg/60"}`}>
            <div className="flex items-center justify-between gap-2">
              <span className="font-mono text-[12px] font-semibold text-ink-1">{r.version}</span>
              {r.projected && <span className="rounded-full bg-line px-1.5 py-0.5 text-[9px] font-medium uppercase tracking-wide text-ink-3">{projectedWord}</span>}
            </div>
            <div className="mt-2 font-mono text-[22px] font-bold leading-none" style={{ color: r.projected ? MUTED : SOL }}>{r.auc.toFixed(2)}</div>
            <div className="mt-0.5 text-[10px] text-ink-3">AUC</div>
            <div className="mt-2 text-[11px] text-ink-2">{r.labels.toLocaleString("es-MX")} <span className="text-ink-3">{labelsWord}</span></div>
          </div>
