...
// Nombre y headcount dejan de estar hardcodeados ("Tijuana Norte / 1,180") para
// que el demo use el contexto del prospecto y los conteos cuadren con la población.
import type { PlantProfile } from "@/types";

export const DEFAULT_HEADCOUNT = 1180;
const DEFAULT_NAME = "Planta demo";

let stored: { name: string; headcount: number; updatedAt: string } | null = null;
...
export async function setPlantProfile(input: { name: string; headcount: number }): Promise<PlantProfile> {
  stored = { name: input.name, headcount: input.headcount, updatedAt: new Date().toISOString() };
